...
# subset the data to the country and metric
...
#fig = px.line(df_gdp_o, x = "year", y = "value", color = "country", 
 #             title = title, labels={"value": "GDP Percap"})
#st.plotly_chart(fig)

#st.markdown("This plot show the GDP Per Capita for countries in Oceania")
...
# subset the data to the country and metric
#continent_list = list(df['continent'].unique()) # list of unique continents
#continent = st.selectbox(label = "Choose a continent", options = continent_list)
#metric_list = list(df['metric'].unique))
#metric
#metric = "pop"
#query = f"continent=='{continent}' & metric=='{metric}'" # this is formatting of the string
#df_filtered = df.query(query) # filters to only the rows that meet our query


# create plot of data
#title = "GDP for countries in Oceania"
#fig = px.line(df_filtered, x = "year", y = "value", color = "country", 
#              title = title, labels={"value": "GDP Percap"}) # remember it has to find value and then drop it in there
...
# create plot of data
#metric_lables = {"gdpPercap":"GDP Per Capita","lifeExp":"Average Life Epectancy","pop":"Population"}



# create plot of data
#title = f"{} for GDP for countries in {}"
#fig = px.line(df_filtered, x = "year", y = "value", color = "country", 
#              title = title, labels={"value": "GDP Percap"})
#st.plotly_chart(fig)
...
# app.py after we select the countries with multiselect
...
# app.py after we've added years slider
